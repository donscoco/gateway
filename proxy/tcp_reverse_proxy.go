...
func NewTcpLoadBalanceReverseProxy(c *middleware_tcp.TcpSliceRouterContext, lb load_balance.LoadBalance) *TcpReverseProxy {
	return func() *TcpReverseProxy {
		...
		return &TcpReverseProxy{
			ctx:             c.Ctx,
			Addr:            nextAddr,                                       // fixme: 这里不能写死，要负载均衡，每次都要动态获取新的addr
			Director:        func() string { s, _ := lb.Get(""); return s }, // domark:rm
...
type TcpReverseProxy struct {
	...
	Addr                 string
	Director             func() string // domark:rm
...
func (dp *TcpReverseProxy) ServeTCP(ctx context.Context, src net.Conn) {
	...
	dp.Addr = dp.Director() // domark:rm
	log.Println(dp.Director())
...
func (dp *TcpReverseProxy) proxyCopy(errc chan<- error, dst, src net.Conn) {
	_, err := io.Copy(dst, src) // 这里直到关闭前都不往下走
	log.Println("tcp proxy close:", err)
	errc <- err
}
