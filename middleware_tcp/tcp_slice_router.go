...
type TcpSliceRouterHandler struct {
	coreFunc func(*TcpSliceRouterContext) server_tcp.TCPHandler
	router   *TcpSliceRouter
	//myCoreFunc func(ctx context.Context, conn net.Conn)
}
...
func (w *TcpSliceRouterHandler) ServeTCP(ctx context.Context, conn net.Conn) {
	...
	//if w.myCoreFunc != nil {
	//	c.handlers = append(c.handlers,func(c *TcpSliceRouterContext) {w.myCoreFunc(ctx, conn)})
	//}
	c.Reset()
	c.Next()
}
